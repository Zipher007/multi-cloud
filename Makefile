# Copyright 2019 The SodaFoundation Authors.
#
...
file: prebuild
	CGO_ENABLED=0 GOOS=linux go build -ldflags '-w -s -extldflags "-static"' -o $(BUILD_DIR)/file github.com/sodafoundation/multi-cloud/file/cmd

block: prebuild
	CGO_ENABLED=0 GOOS=linux go build -ldflags '-w -s -extldflags "-static"' -o $(BUILD_DIR)/block github.com/sodafoundation/multi-cloud/block/cmd
